    ...
    if request.method == 'POST' and form.is_valid():
        ...
        if user is not None:
            ...
            if user.is_staff or user.is_superuser:
                ...
                messages.error(request, 'Invalid admin credentials. This account is not an admin.')
        else:
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        if user is not None:
            ...
            if is_customer(user):
                ...
                messages.error(request, 'Invalid customer credentials. Please use your customer account.')
        else:
            messages.error(request, 'Invalid username or password.')
...
def add_to_cart_view(request, pk):
    # Update product_ids cookie as before
    if 'product_ids' in request.COOKIES:
        product_ids = request.COOKIES['product_ids']
        if product_ids == "":
            product_ids = str(pk)
    ...
    response = redirect('home')  # Use your homepage URL name here
...
def payment_success_view(request):
    ...
    for product in products:
        models.Orders.objects.get_or_create(
            ...
            status='Pending', email=email, mobile=mobile, address=address)
